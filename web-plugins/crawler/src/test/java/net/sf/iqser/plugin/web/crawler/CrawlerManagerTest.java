...
import net.sf.iqser.plugin.web.crawler.CrawlerManager;
	...
	protected void setUp() throws Exception {
		...
		try {
			...
			stmt.executeUpdate("INSERT INTO providers VALUES(" +
					"DEFAULT, 'net.sf.iqser.plugin.web.html', 'http://www.designerfashion.de', " +
	...
	public void testStartAllCrawler() {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(
					"SELECT Count(url) FROM documents WHERE provider='net.sf.iqser.plugin.web.html'");
			assertTrue(rs.first());
			assertTrue(rs.getInt("Count(url)") > 0);
			
			rs = stmt.executeQuery(
					"SELECT url FROM documents WHERE provider='net.sf.iqser.plugin.web.html'");	
			while (rs.next()) {
